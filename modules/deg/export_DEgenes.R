deg.exportDEGenes <- function(fitted, df_annot = NA, outDirSuffix="", extractCols=c()){
  ...
  if(all(is.na(df_annot))){
    for(resData in fitted@list_signifTest){
      saveCSV(resData@df_degenes, filedir = filedir, filename = paste0("DEgenes_", resData@contrast))
    }
  }else{
    ...
    for(resData in fitted@list_signifTest){
      ...
      saveCSV(df_merged, filedir = filedir, filename = paste0("DEgenes_", resData@contrast))
    }
  }
